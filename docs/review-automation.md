...
- **Decline & close PR** requires a preset or custom message. It locks the link, posts a polite request-changes review pinned to the checked commit, checks the revision again, and closes the PR. The review includes both the selected reason and any additional note, thanks the applicant, and invites a new PR after addressing the feedback. The message is visible in the PR conversation.
